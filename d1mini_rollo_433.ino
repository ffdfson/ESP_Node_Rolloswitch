...
const char* progversion  = "Rollo Schlafzimmer / Kueche";
//----------------------------------------------------------------------------------------------
#define ARDUINO_HOSTNAME  "rollo-esp" //http://rollo.wg/
//----------------------------------------------------------------------------------------------
//web-time
...
const char* rollo_KEY =   "F011F00QQ0F0000";
const char* rollo_UP =    "F011F00QQ0F0000F0F1Q";//3x(oder 3x mehr) + 6x F0F1Q
const char* rollo_DOWN =  "F011F00QQ0F0000F0110";//3x(oder 3x mehr) + 6x F0110
const char* rollo_STOP =  "F011F00QQ0F0000FFFFF";//3x
const char* rollo_KEY2 =  "FFQFF001FFFF1F0";
const char* rollo_UP2 =   "FFQFF001FFFF1F0F0F0F";//3x(oder 3x mehr) + 6x F0F1Q
const char* rollo_DOWN2 = "FFQFF001FFFF1F0F0101";//3x(oder 3x mehr) + 6x F0110
const char* rollo_STOP2 = "FFQFF001FFFF1F0FFFFF";//3x
const char* rollo_KEY3 =  "Q01F10Q1Q0FF00F";
const char* rollo_UP3 =   "Q01F10Q1Q0FF00FF0F1Q";//3x(oder 3x mehr) + 6x F0F1Q
...
#define RolloBefehl_UP2  4
#define RolloBefehl_DOWN2  5
#define RolloBefehl_STOP2  6
#define RolloBefehl_UP3  7
#define RolloBefehl_DOWN3  8
#define RolloBefehl_STOP3  9
...
*/
            ...
            if(tage & maske ){//Serial.print(" isday,");
              if(ntp_stunde==t_st && ntp_minute==t_min){
                  ...
                  if(befehl=="UP2"){Serial.println(" >UP2");  
                      RolloBefehl(4, 1);
                    }
                    else
                  if(befehl=="STOP2"){Serial.println(" >STOP2");  
                      RolloBefehl(6, 1);
                    }
                    else
                  if(befehl=="DOWN2"){Serial.println(" >DOWN2");  
                      RolloBefehl(5, 1);
                    }
                  if(befehl=="UP3"){Serial.println(" >UP3");  
                      RolloBefehl(7, 1);
                    }
                    else
                  if(befehl=="STOP3"){Serial.println(" >STOP3");  
                      RolloBefehl(9, 1);
                    }
                    else
                  if(befehl=="DOWN3"){Serial.println(" >DOWN3");  
                      RolloBefehl(8, 1);
                    }    
                    /*else{
                      Serial.print(befehl); 
...
void handleAction() {
  ...
  for (uint8_t i = 0; i < server.args(); i++) {
    ...
    if (server.argName(i) == "rollo") {
      ...
      if (server.arg(i) == "UP2")   rollobefehl = 4;
      if (server.arg(i) == "STOP2") rollobefehl = 6;
      if (server.arg(i) == "DOWN2") rollobefehl = 5;
      if (server.arg(i) == "UP3")   rollobefehl = 7;
      if (server.arg(i) == "STOP3") rollobefehl = 9;
      if (server.arg(i) == "DOWN3") rollobefehl = 8;
    }
    if (server.argName(i) == "rkey") {
      if (server.arg(i) == rollo_KEY or rollo_KEY2 or rollo_KEY3)rollokeyOK = 1;
    }
  }
  message += "\n ]";
...
uint8_t RolloBefehl(uint8_t befehl, uint8_t key) {
  ...
  if (key == 1) {
    ...
    if (befehl == 4) {
      sendQuadState(rollo_UP2, rolloWiederholungen);
      re = 1;
    }
    if (befehl == 6) {
      sendQuadState(rollo_STOP2, rolloWiederholungen); //3x
      re = 2;
    }
    if (befehl == 5) {
      sendQuadState(rollo_DOWN2, rolloWiederholungen);
      re = 3;
    }
    if (befehl == 7) {
      sendQuadState(rollo_UP3, rolloWiederholungen);
      re = 1;
    }
    if (befehl == 9) {
      sendQuadState(rollo_STOP3, rolloWiederholungen); //3x
      re = 2;
    }
    if (befehl == 8) {
      sendQuadState(rollo_DOWN3, rolloWiederholungen);
      re = 3;
    }
  }
  return re;
